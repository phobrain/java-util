...
/**
  ...
  *     .pairs files 
  *
  *  Files' Format: id0 id1 val01 val10
  */

public class PairFilesReader extends Stdio {

    final static int READAHEAD_QUEUE_SIZE = 512;

    static int readThreads;
    ...
    private PairFilesReader() {
        readThreads = 5; // Runtime.getRuntime().availableProcessors() - 3;
        if (readThreads < 1) {
            readThreads = 1;
        }
    ...
    synchronized public int indexFile(String fname) {
        if (initted) {
            err("Too late! You initted!!! No adding (or even looking up.. add?)");
        }
    ...
    public boolean next(int lineNum, String id0, String id1) 
            throws Exception {
        ...
        for (int i=0; i<pvQs.length; i++) {
            ...
            if (pv0.id0 == null  ||  pv0.id1 == null) { 
                pout("returning false on null id(s) line " + pv0.lineNum);
                return false;
            }

            if (!pv0.id0.equals(id0)  ||  !pv0.id1.equals(id1)) {
                throw new Exception("PairFilesReader.next():" + 
...
if (in_lines[i] == null) err("in line null at " + i);
        ...
        pv0.id0 = split_lines[0][0];
        pv0.id1 = split_lines[0][1];

        // make other pvs, checking internal id0,id1 consistency with pv0
        ...
        for (int i=1; i<pvs.length; i++) {
            ...
            String err = "";
            if (!pv0.id0.equals(split_lines[i][0])) {
                err = "id0";
            }
            if (!pv0.id1.equals(split_lines[i][1])) {
                err += " id1";
            }
            if (!"".equals(err)) {
            ...
            pv.id0 = split_lines[i][0];
            pv.id1 = split_lines[i][1];
